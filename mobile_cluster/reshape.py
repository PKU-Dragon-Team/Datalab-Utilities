...
for i in range(len(data)):
    ...
    try:
        ...
        dset[(row.x, row.y)] = {'location': row.location, 'usr_count': [0] * 2 * 24}
...
with connection.cursor() as cursor:
    ...
    for key, val in dset.items():
        if n < 100:
            value_list.append(value_template % (val['location'], json.dumps([int(x) for x in val['usr_count']]), key[0], key[1]))
            n += 1
        else:
